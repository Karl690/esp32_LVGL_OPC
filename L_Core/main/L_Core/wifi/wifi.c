	...
	void* event_data)
{
	if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
		...
		systemconfig.wifi.status = true;
		ip_event_got_ip_t* event = (ip_event_got_ip_t*) event_data;
		memcpy(&wifi_info, &event->ip_info, sizeof(esp_netif_ip_info_t));
		sprintf(ipAddress, "%d.%d.%d.%d", IP2STR(&event->ip_info.ip));
...
void wifi_connect()
{
	...
	strcpy((char*)wifi_config.sta.ssid, (char*)systemconfig.wifi.ssid);
	strcpy((char*)wifi_config.sta.password, (char*)systemconfig.wifi.password);
